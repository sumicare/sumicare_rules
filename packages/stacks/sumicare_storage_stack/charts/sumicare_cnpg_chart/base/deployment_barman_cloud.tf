...
resource "kubernetes_deployment" "cnpg_barman_cloud" {
  metadata {
    name      = "${local.app_name}-barman-cloud"
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account.plugin_barman_cloud.metadata[0].name

        container {
          name  = "barman-cloud"
          image = "${var.image}:${var.cnpg_version}"
