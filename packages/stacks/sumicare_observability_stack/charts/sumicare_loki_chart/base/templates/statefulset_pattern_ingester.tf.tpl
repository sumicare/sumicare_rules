...
resource "kubernetes_stateful_set" "loki_pattern_ingester" {
  metadata {
    name      = "${local.app_name}-pattern-ingester"
    namespace = var.namespace
    labels    = local.pattern_ingester_labels
  }

  spec {
    replicas = var.pattern_ingester_replicas

    selector {
      match_labels = local.pattern_ingester_labels
    }
    ...
    template {
      ...
      spec {
        ...
        {{ NodeAffinityWithPodAntiAffinity "local.pattern_ingester_labels" }}
      }
    }
    ...
    service_name           = "${local.app_name}-pattern-ingester-headless"
