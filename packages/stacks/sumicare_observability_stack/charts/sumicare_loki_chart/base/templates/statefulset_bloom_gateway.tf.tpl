...
resource "kubernetes_stateful_set" "loki_bloom_gateway" {
  metadata {
    name      = "${local.app_name}-bloom-gateway"
    namespace = var.namespace
    labels    = local.bloom_gateway_labels
  }

  spec {
    replicas = var.bloom_gateway_replicas

    selector {
      match_labels = local.bloom_gateway_labels
    }
    ...
    template {
      ...
      spec {
        ...
        {{ NodeAffinityWithPodAntiAffinity "local.bloom_gateway_labels" }}
      }
    }
    ...
    service_name           = "${local.app_name}-bloom-gateway-headless"
