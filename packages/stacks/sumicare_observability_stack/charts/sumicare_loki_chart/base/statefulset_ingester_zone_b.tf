...
resource "kubernetes_stateful_set" "loki_ingester_zone_b" {
  metadata {
    name      = "${local.app_name}-ingester-zone-b"
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          args  = ["-config.file=/etc/loki/config/config.yaml", "-target=ingester", "-ingester.availability-zone=zone-b"]
